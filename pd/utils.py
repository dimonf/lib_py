...
class RSet:
    def __init__(self, file, data_source_type='1c-cy', filter=None, dtype=None):
	...
	#TODO: move "filter" and "lowercase" to __init__ arguments
        ...
        self.dt_source = self.import_data(filter=filter, dtype=dtype)
    ...
    def import_data(self, filter=None, dtype=None):
        if self.data_source_type == '1c-cy':
            tmp_dt = pd.read_csv(self.file_path, header=0,sep="\t",encoding='windows-1251',dtype=dtype,
                               compression='infer')
            if filter:
