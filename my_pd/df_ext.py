...
import fnmatch, types, pandas as pd
...
def _drill(self, search_val, df_source = None):
    ...
    if type(df_source) == pd.core.frame.DataFrame:
        recs = df_source.query(query_spec)
    elif not hasattr(self,'_source_df'):
        raise TypeError('no source dataframe defined')
    else:
...
def _pivot_table(self, values=None, index=None, columns=None, aggfunc='sum',
    ...
    t_df.drild = types.MethodType(_drill, t_df)
    #t_df.drill = _drill
    return t_df
